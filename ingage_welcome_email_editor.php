<?php
/*
Plugin Name: Ingage Welcome Email Editor
Plugin URI: https://ingage.co
Description: Forked from SB Welcome Email Editor
Version: 1.0.0
Author: Ingage
Author URI: https://ingage.co
Text Domain: ingage-wee
...
define('ING_WE_PRODUCT_NAME', 'Ingage Welcome Email');
define('ING_WE_PLUGIN_DIR_PATH', $sb_we_dir);
define('ING_WE_PLUGIN_DIR_URL', trailingslashit(str_replace(str_replace('\\', '/', ABSPATH), $sb_we_home, $sb_we_dir)));
define('ING_WE_PLUGIN_DIRNAME', str_replace('/plugins/', '', strstr(ING_WE_PLUGIN_DIR_URL, '/plugins/')));

/* LS 10/11/2015 */
$sb_we_display_name = apply_filters('sb_we_display_name', ING_WE_PRODUCT_NAME);
$sb_we_capability = apply_filters('sb_we_capability', 'manage_options');
// define( 'ING_WE_DOMAIN' , 'sb-we' );
// add_action('plugins_loaded', 'sb_we_load_textdomain');
// function sb_we_load_textdomain() {
// 	load_plugin_textdomain( ING_WE_DOMAIN, false, dirname( plugin_basename( __FILE__ ) ) . '/languages/' );
// }
/* LS 10/11/2015 */
...
function sb_we_loaded()
{
		...
		if (version_compare(get_bloginfo('version'), '4.3') >= 0) { //only run when the version is on or past 4.3
			if (!get_option('sb_we_v4_migration_complete')) {
				...
				if ($settings->header_send_as == 'text') {
					$replace_to = __('Visit [reset_pass] to set your password', ING_WE_DOMAIN);
				}
	...
	if (is_admin() && !empty($_REQUEST['_wp_http_referer'])) {
		if (!$sb_we_active) {
			$msg = '<div class="error sbwe-error"><p>' . $sb_we_display_name . ' ' . __('can not function because another plugin is conflicting. Please disable other plugins until this message disappears to fix the problem.', ING_WE_DOMAIN) . '</p></div>';
...
function sb_we_init()
{
	...
	if (!$sb_we_settings = get_option('sb_we_settings')) {
		...
		$sb_we_settings->user_subject = __('[[blog_name]] Your username and password', ING_WE_DOMAIN);
		$sb_we_settings->user_body = __('Username: [user_login]<br />Password: [user_password]<br />[login_url]', ING_WE_DOMAIN);
		$sb_we_settings->admin_subject = __('[[blog_name]] New User Registration', ING_WE_DOMAIN);
		$sb_we_settings->admin_body = __('New user registration on your blog [blog_name]<br /><br />Username: [user_login]<br />Email: [user_email]', ING_WE_DOMAIN);
	...
	function wp_new_user_notification($user_id, $depracated = '', $notify = '')
	{
		...
		if ($user = new WP_User($user_id)) {
			...
			if ($notify && $notify != 'admin') { //needs to be like this for backwards compatibility
				...
				$user_message = str_replace('[reset_pass_link]', '<a href="' . $reset_pass_url . '" target="_blank">' . __('Click to set', ING_WE_DOMAIN) . '</a>', $user_message);
...
function sb_we_update_settings()
{
	...
	if ($post_settings = sb_we_post('settings')) {
		...
		if (update_option('sb_we_settings', $settings)) {
			sb_we_display_message(__('Settings have been successfully saved', ING_WE_DOMAIN));
		}
	}
}
...
function sb_we_settings()
{
	...
	if (sb_we_post('test_send')) {
		...
		sb_we_display_message(__('Test email sent to', ING_WE_DOMAIN) . ' "' . $current_user->user_email . '"');
	}
	...
	$page_options = array(
		'general_settings_label' => array(
			'title' => __('General Settings', ING_WE_DOMAIN),
			'type' => 'label',
			'style' => 'width: 500px;',
			'description' => __('These settings effect all of this plugin and, in some cases, all of your site.', ING_WE_DOMAIN)
		),
		'settings[header_from_email]' => array(
			'title' => __('From Email Address', ING_WE_DOMAIN),
			'type' => 'text',
			'style' => 'width: 500px;',
			'description' => __('Global option change the from email address for all site emails', ING_WE_DOMAIN)
		),
		'settings[header_from_name]' => array(
			'title' => __('From Name', ING_WE_DOMAIN),
			'type' => 'text',
			'style' => 'width: 500px;',
			'description' => __('Global option change the from name for all site emails', ING_WE_DOMAIN)
		),
		'settings[header_send_as]' => array(
			'title' => __('Send Email As', ING_WE_DOMAIN),
			'type' => 'select',
			'style' => 'width: 100px;',
			'options' => array(
				'text' => __('TEXT', ING_WE_DOMAIN),
				'html' => __('HTML', ING_WE_DOMAIN)
			),
			'description' => __('Send email as Text or HTML (Remember to remove html from text emails).', ING_WE_DOMAIN)
		),
		'settings[set_global_headers]' => array(
			'title' => __('Set Global Email Headers', ING_WE_DOMAIN),
			'type' => 'yes_no',
			'style' => 'margin-left: 10px;',
			'description' => __('This is one of those "hit it with a hammer" type functions to set to yes when you might be having issues with the from name and address setting. Or setting it to no as and when another plugin is being effected by Welcome Email Editor\'s existence.', ING_WE_DOMAIN)
		),
		'welcome_email_settings_label' => array(
			'title' => __('Welcome Email Settings', ING_WE_DOMAIN),
			'type' => 'label',
			'style' => 'width: 500px;',
			'description' => __('These settings are for the email sent to the new user on their signup.', ING_WE_DOMAIN)
		),
		'settings[user_subject]' => array(
			'title' => __('User Email Subject', ING_WE_DOMAIN),
			'type' => 'text',
			'style' => 'width: 500px;',
			'description' => __('Subject line for the welcome email sent to the user.', ING_WE_DOMAIN)
		),
		'settings[user_body]' => array(
			'title' => __('User Email Body', ING_WE_DOMAIN),
			'type' => 'textarea',
			'style' => 'width: 650px; height: 500px;',
			'description' => __('Body content for the welcome email sent to the user.', ING_WE_DOMAIN)
		),
		'settings[we_attachment_url]' => array(
			'title' => __('Attachment URL', ING_WE_DOMAIN),
			'type' => 'text',
			'style' => 'width: 500px;',
			'description' => __('If you want the welcome email to have an attachment then put the URL here. The file MUST be on THIS server in a web servable directory. If you don\'t understand this then use the WordPress media uploader and paste the FULL URL into this box and it will do the rest.', ING_WE_DOMAIN)
		),
		'settings[header_additional]' => array(
			'title' => __('Additional Email Headers', ING_WE_DOMAIN),
			'type' => 'textarea',
		...
		'settings[header_reply_to]' => array(
			'title' => __('Reply To Email Address', ING_WE_DOMAIN),
			'type' => 'text',
			'style' => 'width: 500px;',
			'description' => __('Optional Header sent to change the reply to address for new user notification.', ING_WE_DOMAIN)
		),
		'welcome_email_admin_settings_label' => array(
			'title' => __('Welcome Email Admin Notification Settings', ING_WE_DOMAIN),
			'type' => 'label',
			'style' => 'width: 500px;',
			'description' => __('These settings are for the email sent to the admin on a new user signup.', ING_WE_DOMAIN)
		),
		'settings[admin_subject]' => array(
			'title' => __('Admin Email Subject', ING_WE_DOMAIN),
			'type' => 'text',
			'style' => 'width: 500px;',
			'description' => __('Subject Line for the email sent to the admin user(s).', ING_WE_DOMAIN)
		),
		'settings[admin_body]' => array(
			'title' => __('Admin Email Body', ING_WE_DOMAIN),
			'type' => 'textarea',
			'style' => 'width: 650px; height: 300px;',
			'description' => __('Body content for the email sent to the admin user(s).', ING_WE_DOMAIN)
		),
		'settings[admin_notify_user_id]' => array(
			'title' => __('Send Admin Email To...', ING_WE_DOMAIN),
			'type' => 'text',
			'style' => 'width: 500px;',
			'description' => __('This allows you to type in the User IDs of the people who you want the admin notification to be sent to. 1 is admin normally but just add more separating by commas (eg: 1,2,3,4).', ING_WE_DOMAIN)
		),
		'forgot_password_settings_label' => array(
			'title' => __('User Forgot Password Email Settings', ING_WE_DOMAIN),
			'type' => 'label',
			'style' => 'width: 500px;',
			'description' => __('These settings are for the email sent to the user when they use the inbuilt Wordpress forgot password functionality.', ING_WE_DOMAIN)
		),
		'settings[password_reminder_subject]' => array(
			'title' => __('Forgot Password Email Subject', ING_WE_DOMAIN),
			'type' => 'text',
			'style' => 'width: 500px;',
			'description' => __('Subject line for the forgot password email that a user can send to themselves using the login screen. Use [blog_name] where appropriate.', ING_WE_DOMAIN)
		),
		'settings[password_reminder_body]' => array(
			'title' => __('Forgot Password Message', ING_WE_DOMAIN),
			'type' => 'textarea',
			'style' => 'width: 650px; height: 500px;',
			'description' => __('Content for the forgot password email that the user can send to themselves via the login screen. Use [blog_name], [site_url], [reset_url] and [user_login] where appropriate. Note to use HTML in this box only if you have set the send mode to HTML. If not text will be used and any HTML ignored.', ING_WE_DOMAIN)
		),
		'submit' => array(
			'title' => '',
			'type' => 'submit',
			'value' => __('Update Settings', ING_WE_DOMAIN)
		),
		'test_send' => array(
			'title' => '',
			'type' => 'submit',
			'value' => __('Test Emails (Save first, will send to current user)', ING_WE_DOMAIN)
		)
	);

	$page_options = apply_filters('sb_we_settings_fields', $page_options);

	$html .= '<div style="margin-bottom: 10px;">' . __('This page allows you to update the Wordpress welcome email and add headers to make it less likely to fall into spam. You can edit the templates for both the admin and user emails and assign admin members to receive the notifications. Use the following hooks in any of the boxes below: [site_url], [login_url], [reset_pass_url], [user_email], [user_login], [blog_name], [admin_email], [user_id], [custom_fields], [first_name], [last_name], [date], [time], [bp_custom_fields] (buddypress custom fields .. admin only), [post_data] (admin only. Sends $_REQUEST)', ING_WE_DOMAIN) . '</div>';
	$html .= sb_we_start_box(__('Settings', ING_WE_DOMAIN));
...
function sb_we_get_yes_no($name, $value = false, $class = false, $style = false)
{
	...
	$return .= '<span class="sbwe-yesno">' . __('Yes', ING_WE_DOMAIN) . ': </span>' . sb_we_get_input($name, 'radio', 1, $class, $style, ($value == 1 ? 'checked="checked"' : '')) . '<br />';
	$return .= '<span class="sbwe-yesno">' . __('No', ING_WE_DOMAIN) . ': </span>' . sb_we_get_input($name, 'radio', 0, $class, $style, ($value == 1 ? '' : 'checked="checked"'));
